...
st.markdown("""
    ...
    [data-testid="stMetricValue"] { font-size: 2.2rem !important; font-weight: 700 !important; color: #1F2937 !important; }
    
    .stTabs [data-baseweb="tab-list"] { gap: 20px; background-color: transparent; }
    .stTabs [data-baseweb="tab"] {
        height: 50px; background-color: #FFFFFF; border-radius: 12px;
        padding: 10px 30px; font-weight: 600; color: #64748B; border: 1px solid #E2E8F0;
    }
    .stTabs [aria-selected="true"] { background-color: #7B61FF !important; color: white !important; border: none !important; }

    h1, h2, h3 { font-family: 'Inter', sans-serif; color: #1F2937; font-weight: 800; }

    .footer {
        position: fixed; left: 0; bottom: 0; width: 100%;
        background-color: rgba(248, 249, 251, 0.9);
        text-align: center; font-size: 11px; color: #94A3B8; padding: 10px;
...
if df_cdi is not None:
    ...
    with c2:
        # Find peak CDI for display
        peak_val = df_cdi['cdi_score'].max()
        st.metric("Peak Drift", f"{peak_val:.2f}", "Nov 2022")
    with c3:
    ...
    with tab2:
        ...
        # Smart Column Finder to avoid errors
        x_col = df_key.columns[0] # Usually 'month' or 'window_key'
        y_col = 'mention_rate' if 'mention_rate' in df_key.columns else df_key.columns[-1]

        fig2 = px.bar(df_key, x=x_col, y=y_col, color_discrete_sequence=['#FF4B8B'])
        ...
        # CRASH FIX: Removed invalid update_traces call
    ...
    st.error("Data files not found. Ensure 'cdi_results.csv' and 'keyword_frequency_monthly.csv' are in your GitHub repo.")
